...
class VirtualniZupan:
    def __init__(self):
        print("Pripravljam virtualnega župana (verzija 4.2)...")
        ...
        try:
            ...
            print("KRITIČNA NAPAKA: Baze znanja ni mogoče naložiti. Ali ste najprej zagnali 'zgradi_bazo.py'?")
            traceback.print_exc()
    ...
    def pridobi_podatke_o_zaporah(self):
        if not self.nap_access_token:
    ...
    def odgovori(self, uporabnikovo_vprasanje: str):
        ...
        if any(beseda in uporabnikovo_vprasanje.lower() for beseda in kljucne_besede_promet):
            spletni_kontekst = self.pridobi_podatke_o_zaporah()
            if spletni_kontekst:
        ...
        ZELO POMEMBNO PRAVILO: Danes je leto {trenutno_leto}. Tvoja naloga je, da v priloženih informacijah poiščeš najbolj relevanten in ČASOVNO NAJNOVEJŠI podatek.
        - Če najdeš podatek za leto {trenutno_leto}, ga uporabi.
        - Če podatka za {trenutno_leto} ni, lahko uporabiš podatek iz prejšnjih let (npr. {trenutno_leto - 1}), ampak MORAŠ JASNO IN NEDVOUMNO povedati, iz katerega leta je ta informacija.
        - Nikoli ne predstavljaj starih podatkov, kot da so aktualni za danes.
