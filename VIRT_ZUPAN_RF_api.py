...
class VirtualniZupan:
    ...
    def nalozi_bazo(self):
        if self.collection is None:
            try:
                print("Poskušam naložiti bazo znanja...")
    ...
    def preveri_zapore_cest(self):
        ...
        try:
            ...
            # --- TUKAJ JE BIL POPRAVEK: 'description' -> 'opis' ---
            relevantne_zapore = [
                dogodek['properties']
            ...
            if e.response.status_code == 401 or e.response.status_code == 403:
                self.nap_access_token = None # Žeton je potekel ali neveljaven, ponovno ga pridobimo ob naslednjem klicu
                return "Žal mi neposreden vpogled v stanje na cestah trenutno ne deluje (težava z avtorizacijo). Prosim, poskusite znova čez trenutek."
    ...
    def odgovori(self, uporabnikovo_vprasanje: str, session_id: str):
        ...
        if any(beseda in uporabnikovo_vprasanje.lower() for beseda in kljucne_besede_promet):
            odgovor_promet = self.preveri_zapore_cest()
            self.belezi_pogovor(session_id, uporabnikovo_vprasanje, odgovor_promet)
            return odgovor_promet

        if session_id not in self.zgodovina_seje: self.zgodovina_seje[session_id] = []
        ...
        prompt_za_llm = f"""Ti si 'Virtualni župan občine Rače-Fram'. Upoštevaj pretekli pogovor za kontekst.
--- ZGODOVINA POGOVORA ---
{zgodovina_za_prompt}
---
...
TVOJ ODGOVOR:"""
        ...
        if len(self.zgodovina_seje[session_id]) > 3: self.zgodovina_seje[session_id].pop(0)
